...
class FASTHead(nn.Module):
    ...
    def get_results(self, out, img_meta, cfg, scale=2):
        ...
        texts = F.interpolate(out[:, 0:1, :, :],
                              size=(int(img_meta['img_size'][0] // scale), int(img_meta['img_size'][1] // scale)),
                              mode='nearest')  # B*1*320*320
        texts = self._max_pooling(texts, scale=scale)  # B*1*320*320
        score_maps = torch.sigmoid_(texts)  # B*1*320*320
        score_maps = F.interpolate(score_maps,
                                   size=(int(img_meta['img_size'][0] // scale), int(img_meta['img_size'][1] // scale)),
                                   mode='nearest')  # B*1*640*640
        ...
        labels = F.interpolate(labels, size=(int(img_meta['img_size'][0]), int(img_meta['img_size'][1])),
                               mode='nearest')  # B*1*640*640
        ...
        scales = float(org_img_size) / float(img_size)
